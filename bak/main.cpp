...
int g_eventpoll;
struct epoll_event g_event_list[1000];  
...
int main(int argc, char *argv[])
{
    ...
    file = (FILE *)fopen("/opt/inno_srv/conf/monitorserver.conf", "r");
    ...
    */
    ...
    if((g_eventpoll = epoll_create(LIMIT_SIZE)) < 0)
    {
        CMiscUtil::WriteErrorLog("EPOLL CREATE ERROR", -1, 0);
        exit(EXIT_SUCCESS); 
    }

    result = pthread_create(&c_thread, &thread_attr, epoll_thread, NULL);
    if (result != 0)
    {
        CMiscUtil::WriteErrorLog("EPOLL Thread Create Error", -1, 0);
        exit(EXIT_SUCCESS);     
    }
    ...
    while(1)
    {
        ...
        CMiscUtil::WriteLog("Test Log for service", client_sockfd, 0);

        client_len = sizeof(client_address);
        client_sockfd = accept(server_sockfd, (struct sockaddr *)&client_address, (socklen_t *)&client_len);
        CMiscUtil::WriteLog("TRY!!!! PEER MANAGER SOCKET ACCEPT", client_sockfd, 0);
        if( client_sockfd == -1 )
        {
        ...
        CMiscUtil::WriteLog("PEER MANAGER SOCKET ACCEPT", client_sockfd, 0);
        ...
        if( fcntl(client_sockfd, F_SETFL, O_NONBLOCK) < 0 )
        {
            CMiscUtil::WriteErrorLog("Client socket non-blocking mode error", -1, 0);
            close(client_sockfd);
            continue;
        }
        ...
        *sockfd = client_sockfd;
        ...
        {
            CMiscUtil::WriteErrorLog("EPOLL ADD ERROR", -1, 0);
            delete sockfd;
            continue;
        }
    }
...
void *epoll_thread(void *arg)
{
    ...
    while(1)
    {
        nready = epoll_wait(g_eventpoll, g_event_list, LIMIT_SIZE, -1);
        if( nready < 0 )
        {
            CMiscUtil::WriteErrorLog("EPOLL WAIT ERROR", -1, 0);
            continue;
        }

        for( int i=0; i<nready; i++ )
        {
            sockfd = (int *)g_event_list[i].data.ptr;
            ...
            epoll_ctl( g_eventpoll, EPOLL_CTL_DEL, client_sockfd, g_event_list);
            
            ioctl(client_sockfd, FIONREAD, &nread);

            if( nread == 0 ) 
            {   
                CMiscUtil::WriteLog("epoll_thread receive - SOCKET CLOSED-1", client_sockfd, 0);
                delete sockfd;
                continue;
            }

            CMiscUtil::WriteLog("epoll received", client_sockfd, 0);
